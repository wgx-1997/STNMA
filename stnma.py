"""
    Social Trust Network embedding with mixed-step random walk and Adaptive negative sampling(STNMA)
"""
...
class STNMA(object):
    def __init__(self, args):
        ...
        self.out_lf = np.random.rand(self.num_nodes, self.args.dim)
        # Inward latent factor vectors.
        self.in_lf = np.random.rand(self.num_nodes, self.args.dim)
        # Outward trust pattern vectors.
        self.out_tp = np.zeros((self.num_nodes, 16))
        # Inward trust pattern vectors.
        self.in_tp = np.zeros((self.num_nodes, 16))
    ...
    def _calculate_negative_sampling_distribution(self):
        ...
        for walk in self.walks:
            ...
            for start in range(walk_len - 1):
                u = walk[start]
                sign = 1
                context = walk[start + 1: min(start + self.args.window_size + 1, self.args.walk_len * 2)]
                pre_v = u
                path_len = 0
                for v in context:
                    if v == u:
    ...
    def fit(self):
        ...
        for walk in self.walks:
            ...
            for start in range(walk_len - 1):
                ...
                count = 1
                for v in context:
                    if v == u or count > self.args.walk_len:
                        break
                    sign *= self.G[pre_v][v]['polarity']
                    trust = ((self.args.window_size - path_len) / self.args.window_size) ** self.args.m
                    path_len += 1
                    ...
                    rr = np.random.random()
                    pre_v = v
                    if rr > self.args.onestep:
                        continue
                    count += 1
                    triad_uv = np.array(self.Motif.motif_vector(u, v))
                    X = self.out_lf[u] @ self.in_lf[v] + (1 / 2) * (self.out_tp[u] + self.in_tp[v]) @ triad_uv
                    p_uv = sigmoid(sign * X)
                    out_u_g = 0
                    ...
                    #     noise = random.choice(nodes)
                        X = self.out_lf[u] @ self.in_lf[noise]
        ...
        pbar.close()
        W_out = np.zeros((self.num_nodes, self.args.dim + 16))
        W_in = np.zeros((self.num_nodes, self.args.dim + 16))
        ...
        save_path = "%s/%s-%g-onestep%g-" % (self.args.embedding_path, self.args.dataset, self.args.test_size, self.args.onestep)
        np.save(save_path + "out", W_out)
        np.save(save_path + "in", W_in)
